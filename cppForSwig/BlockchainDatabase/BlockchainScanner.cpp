...
void BlockchainScanner::processInputs()
{
   ...
   while (true) {
      ...
      for (auto& spent_txout : batch->spentOutputs_) {
         auto hash_iter = utxoMap_.find(spent_txout.parentHash);
         if (hash_iter == utxoMap_.end()) {
            LOGERR << "missing utxo";
            continue;
         }

         auto utxo_iter = hash_iter->second.find(spent_txout.txOutIndex);
   ...
   while (true) {
      ...
      for (unsigned i = 0; i < txns.size(); i++) {
         ...
         for (unsigned y = 0; y < txn.txouts_.size(); y++) {
            ...
            stxo.parentTxOutCount = txn.txouts_.size();
            stxo.isCoinbase = txn.isCoinbase_;
            ...
            TxIOPair txio{txioKey, value};
            txio.setFromCoinbase(txn.isCoinbase_);
            subssh.txioMap.emplace(txioKey, std::move(txio));
         }
      }
   }
...
void BlockchainScanner::processInputsThread(ParserBatch* batch)
{
   ...
   while (true) {
      ...
      for (unsigned i = 0; i < txns.size(); i++) {
         ...
         for (unsigned y = 0; y < txn.txins_.size(); y++) {
            ...
            auto iter = subssh.txioMap.emplace(
               txoutkey, TxIOPair{txoutkey, stxo.getValue()}).first;
            iter->second.setTxIn(txinkey);

            //add to spentTxOuts_
            spentOutputs.emplace_back(std::move(stxo));
         }
      }
   }
   ...
   for (auto& ssh_pair : sshMap) {
      ...
      for (auto& subssh_pair : ssh_pair.second) {
         ...
         for (auto& txio_pair : subssh_pair.second.txioMap) {
            auto emplaceResult = txio_iter->second.txioMap.emplace(
...
void BlockchainScanner::writeBlockData()
{
   ...
   while (true) {
      ...
      std::thread writeHintsThreadId(writeHintsLambda, batch.get());
      ...
      std::map<BinaryData, BinaryWriter> serializedSubSSH;
      std::map<BinaryData, BinaryWriter> serializedStxo;
      {
         for (auto& ssh : batch->sshMap_) {
            for (auto& subssh : ssh.second) {
               ...
               serializedSubSSH.emplace(
                  subsshkey.getDataRef(), std::move(bw));
            }
         }

         for (const auto& utxomap : batch->outputMap_) {
            for (const auto& utxo : utxomap.second) {
               BinaryWriter bw;
               utxo.second.serializeDBValue(bw);
               auto emplaceIter = serializedStxo.emplace(
                  utxo.second.getDBKey(), std::move(bw));
            }
         }
      }
      ...
      //write data
      {
         ...
         for (const auto& stxo : serializedStxo) {
...

      {
         ...
         sdbi.topBlkHgt = topheader->getBlockHeight();
         sdbi.topScannedBlkHash = topheader->getThisHash().toBinaryData();
...
void BlockchainScanner::updateSSH(bool force, int32_t startHeight)
{
   ...
   //update sdbi
   sdbi.topScannedBlkHash = topScannedBlockHash_;
   sdbi.topBlkHgt = topheight;
